...
- 공통 기능 제공
- 다형성의 기본 구현
- Object는 최상위 부모 클래스이기 때문에 모든 객체는 공통기능을 편리하게 제공(상속)받을 수 있다.  
...
## instanceof 핵심 정리
- instanceof는 **변수 타입**이 아니라 **실제 객체의 타입**을 확인한다.
- obj가 Object 타입으로 선언돼도, 안에 실제로 Dog 객체가 들어있으면 `obj instanceof Dog`는 true가 된다.
- instanceof는 true/false만 반환하며 타입 확인만 한다.
- 자바 16부터 `instanceof`에 패턴 매칭을 적용해 자동으로
